...
We give an example of a closed subset `T` of `ℝ × ℝ`
...
/-- The vertical line segment `{ (0, y) | -1 ≤ y ≤ 1 }`,
which is the set of limit points of `S`
...
/-- A sequence of `x`-values tending to 0 at which the sine curve
has a given `y`-coordinate. -/
